...
export interface RoadRenderContext {
  ...
  roadAnalysisCache: Map<string, ReturnType<typeof analyzeMergedRoad>>;
  gridVersion: number;
}

export interface RoadRenderState {
  roadAnalysisCacheVersion: number;
}

// Helper function to check if a tile has a road
function hasRoadAt(grid: Tile[][], gridSize: number, gridX: number, gridY: number): boolean {
...
function getCachedMergeInfo(
  context: RoadRenderContext,
  state: RoadRenderState,
  gx: number,
  gy: number
): ReturnType<typeof analyzeMergedRoad> {
  const currentVersion = context.gridVersion;
  if (state.roadAnalysisCacheVersion !== currentVersion) {
    context.roadAnalysisCache.clear();
    state.roadAnalysisCacheVersion = currentVersion;
  }

  const key = `${gx},${gy}`;
  let info = context.roadAnalysisCache.get(key);
  if (!info) {
    info = analyzeMergedRoad(context.grid, context.gridSize, gx, gy);
    context.roadAnalysisCache.set(key, info);
  }
  return info;
}
...
export function drawRoad(
  ...
  const { ctx, grid, gridSize, zoom, isMobile, isPanning, isPinchZooming, trafficLightTimer } = context;
  ...
  // Create local hasRoad function that uses context
  const hasRoad = (gridX: number, gridY: number) => hasRoadAt(grid, gridSize, gridX, gridY);
  ...
  const mergeInfo = getCachedMergeInfo(context, state, gridX, gridY);
  ...
  if (currentZoom >= LANE_MARKINGS_MIN_ZOOM) {
    ...
    if (isIntersection && currentZoom >= TRAFFIC_LIGHT_MIN_ZOOM && !skipTrafficLights) {
      const trafficTime = trafficLightTimer;
